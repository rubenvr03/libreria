...
<section class="books grid grid-cols-4 gap-4 container mx-auto my-4 ">
    @forelse ($allBooks as $book )
        <div class="book shadow flex flex-wrap text-center justify-center flex-col dark:bg-gray-800">
            ...
            <p class="mb-3">Disponibilidad: {{$book->disponibilidad == 1 ? 'Disponible' : 'No Disponible'}}</p>
            <p class="mb-3">Año de Publicacion: {{$book->anio_publicacion}}</p>
            <div class="buttons flex gap-2 justify-center">
                <a class="rounded-full bg-orange-200 p-2 my-2 " href="{{route('actualizarLibro', $book->id)}}">Modificar Libro</a>
